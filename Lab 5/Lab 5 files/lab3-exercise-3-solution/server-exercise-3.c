...
int main()
{
    ...
    while (1)
    {
        ...
        if (m.msg_type == 0)
        {
            ...
            n_chars++;
            char_data[n_chars].ch = 0;
            char status[50] = "ok";
            zmq_send(responder, status, 50, 0);
        }
        ...
        {
            zmq_send(responder, publish_topic, sizeof(publish_topic), 0);
        }
        ...
        zmq_send(publisher, publish_topic, sizeof(publish_topic), ZMQ_SNDMORE);
